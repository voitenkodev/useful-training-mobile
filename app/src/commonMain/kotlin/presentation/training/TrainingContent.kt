...
fun TrainingContent(vm: TrainingViewModel) {
    ...
    Column(
        modifier = Modifier.padding(Design.dp.padding), verticalArrangement = Arrangement.spacedBy(Design.dp.padding)
    ) {
        Header(
            title = "Exercises!", save = {
...
) = Box(modifier = modifier.fillMaxWidth().height(128.dp).dashedBorder(
    width = 2.dp, color = Design.colors.accent_secondary.copy(alpha = 0.5f), shape = Design.shape.default, on = 8.dp, off = 8.dp
).clickable(onClick = onClick), content = {
    TextFieldH2(
        modifier = Modifier.align(Alignment.Center), text = "Add Exercise", color = Design.colors.accent_secondary
    )
})